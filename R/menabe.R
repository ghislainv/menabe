...
# Combining datasets
crop_annual <- fao_annual %>%
  dplyr::mutate(Exports=round(comtrade_annual$Exports))

# Select specific years
cropstats <- crop_annual %>%
  dplyr::filter(Year %in% c(2000, 2005, 2010, 2015, 2017))
write_csv(crop_stats, "manuscript/tables/cropstats.csv")

# Plot production peanut
p <- ggplot(crop_annual %>% filter(Crop=="Peanut"), aes(Year, Production/1000)) + 
  geom_line(col="brown") + 
  scale_x_continuous(name="Year", limits=c(2000, 2017),
...
# Plot exports
p <- ggplot(crop_annual, aes(Year, Exports/1000, col=Crop)) + 
